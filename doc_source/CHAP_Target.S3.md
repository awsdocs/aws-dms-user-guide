...
+ `DatePartitionEnabled` – Specifies partitioning based on dates\. Set this Boolean option to `true` to partition S3 bucket folders based on transaction commit dates\. The default value is `false`\. 
...
In addition, AWS DMS names the transaction files it creates on the Amazon S3 target using a time stamp according to the naming convention following\.

```
CDC_TXN-timestamp.csv
```

Here, `timestamp` is the time when the transaction file was created, as in the example following\. 
...
|   `maxFileSize`   |  A value that specifies the maximum size \(in KB\) of any \.csv file to be created while migrating to S3 target during full load\. Default value: 1,048,576 KB \(1 GB\) Valid values: 1–1,048,576 Example: `maxFileSize=512`  | 
