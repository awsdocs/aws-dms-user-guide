...
+ `ParallelApplyBufferSize` – Specifies the maximum number of records to store in each buffer queue for concurrent threads to push to a Kinesis, Apache Kafka, Elasticsearch, or Amazon Redshift target endpoint during a CDC load\. The default value is 50\. Use this option when `ParallelApplyThreads` specifies more than one thread\. 
